...
def getTreePoss(forestRange_x, forestRange_y, nTrees, algo="uniform", seed=1234):
    ...
    if algo.lower()=="uniform":
        ...
        treePoss = rng.uniform(size=[nTrees,2])
    else:
...
def generateSampleForestIR(fs, nTrees, posSrc, micPos, forestRange_x=None, forestRange_y=None):
    fs = fs
    dur = 1
    sLs = fs*dur
    ir = simulateForestIR(
        nTrees=nTrees,
        posSrc=posSrc,
        forestRange_x=forestRange_x,
        forestRange_y=forestRange_y,
        micPoss=micPos,
        fs=fs,
        sigLen_in_samples=sLs)
    return ir

def generateMicSignals(ir, source):
    x = source
    N = len(x)
    M = len(ir)
    C = ir.shape[1]
    fft_length = N + M - 1
    IR = rfft(ir, n=fft_length, axis=0)
    X = rfft(x, n=fft_length)
    Xrep = np.repeat(X[:, np.newaxis], C, axis=1)
    conv = Xrep * IR
    out = irfft(conv, axis=0)
    return out